...
class TD3_Trainer():
    def __init__(self, replay_buffer, state_space, action_space, hidden_dim, q_lr, policy_lr,\
        ...
        self.side_q_actions = 3 # 3 discrete action choice for gripper control: open, close, remain

        self.q_net1 = QNetwork(state_space, action_space, hidden_dim)
        self.q_net2 = QNetwork(state_space, action_space, hidden_dim)
        self.side_q = DiscreteQNetwork(state_space, hidden_dim, output_dim=self.side_q_actions, machine_type=machine_type)  
        self.target_side_q = DiscreteQNetwork(state_space, hidden_dim, output_dim=self.side_q_actions, machine_type=machine_type)
        ...
        self.target_side_q = self.target_ini(self.side_q, self.target_side_q)
        ...
        self.q_optimizer3 = SharedAdam(self.side_q.parameters(), lr=q_lr)
    ...
    def update(self, batch_size, eval_noise_scale, reward_scale=10., gamma=0.9, soft_tau=1e-2):
        ...
        # update side Q net via DQN manner
        side_q = self.side_q(state)
        side_q_sa = side_q.gather(1, side_action.unsqueeze(1))
        target_side_q = self.target_side_q(next_state).max(1)[0]
        target_side_q_sa = reward + (1-done) * gamma * target_side_q
        q_value_loss3 = ((side_q_sa - target_side_q_sa.detach())**2).mean()

        if torch.isnan(q_value_loss3): # error captur
            print('Error: q loss 3 value is nan')
            # breakpoint()
        else:
            self.q_optimizer3.zero_grad()
            q_value_loss3.backward()
            self.q_optimizer3.step()
        ...
        if self.update_cnt%self.policy_target_update_interval==0:
            ...
            self.target_side_q=self.target_soft_update(self.side_q, self.target_side_q, soft_tau)
    ...
    def save_model(self, path):
        ...
        torch.save(self.side_q.state_dict(), path+'_q3')
    ...
    def load_model(self, path):
        ...
        self.side_q.load_state_dict(torch.load(path+'_q3', map_location=device))
        ...
        for eps in range(max_episodes): # training loop
            ...
            for step in range(max_steps):
                if frame_idx > explore_steps:
                    ...
                    side_action = np.random.randint(0, 3)
                try:
